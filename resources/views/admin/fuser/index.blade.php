                                ...
                                <tbody>
                                    @foreach ($users as $key => $user)
                                        <tr>
                                            <td>{{ ++$key }}</td>
                                            <td>
                                                <img class="w-80" src="{{ asset('profilepic/' . $user->profilepic) }}"
                                                    alt="">
                                            </td>
                                            <td>{{ $user->name }}</td>
                                            ...
                                            <td>
                                                <form id="statusForm{{ $user->id }}" class="d-inline-block"
                                                    action="{{ route('admin.front_user.status_update') }}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="user_id" value="{{ $user->id }}">
                                                    <select
                                                        class="form-control form-control-sm
                                        @if ($user->email_verified == '0') bg-warning
                                        ...
                                        "
                                                        ...
                                                        <option value="0"
                                                            {{ $user->email_verified == '0' ? 'selected' : '' }}>Unverify
                                                        </option>
                                                        <option value="Yes"
                                                            {{ $user->email_verified == 'Yes' ? 'selected' : '' }}>Verify
                                                        </option>
                                                    </select>
                                                </form>
                                            </td>

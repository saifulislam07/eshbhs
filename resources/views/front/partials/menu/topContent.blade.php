<div class="col-sm-auto col-12 no-print">
    <div class="top-left-content">
        <div class="language-change">
            <p
                class="name
            @if (request()->path() == '/') @if ($commonsetting->theme_version == 'theme1')
...
@if ($commonsetting->is_video_hero == '1') text-white @endif
            ...
            <div class="language-menu">
                @foreach ($langs as $lang)
                    <a href="{{ route('changeLanguage', $lang->code) }}"
                        class="{{ $lang->code == $currentLang->code ? 'active' : '' }}">{{ $lang->name }}</a>
                @endforeach
            </div>
        </div>
            ...
            ">
                ...
                <div class="language-menu">
                    @foreach ($currencies as $currency)
                        <a href="{{ route('changeCurrency', $currency->id) }}"
                            class="{{ $currency->id == $currentCurrency->code ? 'active' : '' }}">{{ $currency->name }}</a>
                    @endforeach
                </div>
            </div>
        @endif

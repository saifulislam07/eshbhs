...
<div class="user-menu">
    ...
    @elseif(request()->is('user/product-order/*')) active @endif"
                    href="{{ route('user.product.order') }}"> {{ __('Product Order') }} </a>
            </li>
        @endif
        @if ($visibility->is_shop == 1)
            <li>
                <a class="@if (request()->path() == 'user/downloadable') active @endif"
                    href="{{ route('user.product.downloadable') }}"> {{ __('Downloadable') }} </a>
            </li>
        @endif
        <li>
